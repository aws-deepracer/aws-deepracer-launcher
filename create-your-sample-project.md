...
The AWS DeepRacer device software is now open sourced to enable quick prototyping and development of your own customized robotic applications by re-using or taking inspiration from the existing AWS DeepRacer core application, sample projects or community projects. The AWS DeepRacer core application and the sample projects are built as a collection of **Robotic Operating System (ROS)** packages and nodes running on the base **Ubuntu 20.04 Focal Fossa** image. The DeepRacer device comes with all the pre-requisite packages and libraries installed for building and running new applications developed on ROS2 Foxy distribution. More details about pre-installed set of packages and libraries on the DeepRacer, and installing required build systems can be found in the [Getting Started](https://github.com/awsdeepracer/aws-deepracer-launcher/blob/main/getting-started.md) section of the AWS DeepRacer Open source page. A detailed list of tutorials with step-by-step instructions to build skills in ROS2 can be found [here](https://docs.ros.org/en/foxy/Tutorials.html).
...
Depending on how your sample project will be leveraging the existing AWS DeepRacer core application, you may be required to stop the existing DeepRacer core service using this [guide](https://github.com/awsdeepracer/aws-deepracer-launcher/blob/main/getting-started.md#startstop-the-deepracer-core-service). It is important to stop the service if your sample project reuses a node from the AWS DeepRacer core application which could conflict with the functionality implemented in it. 
...
1. **Open up a terminal on the DeepRacer device (or [SSH into the DeepRacer device](https://github.com/aws-deepracer/aws-deepracer-launcher/blob/main/getting-started.md#optional-use-ssh-to-remotely-connect-to-the-deepracer-device))**: Follow the below instructions to open an Ubuntu terminal window on the DeepRacer device:
    ...
    d. In **Password**, enter the device SSH password. If this is your first time to log in to the device, enter `deepracer` in **Password**. Reset the password, as required, before moving to the next step. You'll use the new password for future logins. For security reasons, use a complex or strong password phrase for the new password.
